...
typedef struct tag_FATFILESYSTEM
{
  FILESYSTEM     Header;
  LPPHYSICALDISK Disk;
  FAT32MBR       Master;
...
static LPFATFILESYSTEM NewFATFileSystem (LPPHYSICALDISK Disk)
{
  LPFATFILESYSTEM This;

  This = (LPFATFILESYSTEM) KernelMemAlloc(sizeof(FATFILESYSTEM));
  if (This == NULL) return NULL;

  MemorySet(This, 0, sizeof(FATFILESYSTEM));

  This->Header.ID         = ID_FILESYSTEM;
  This->Header.References = 1;
  This->Header.Next       = NULL;
  This->Header.Prev       = NULL;
  This->Header.Driver     = &FATDriver;
  This->Disk              = Disk;
  This->FATStart          = 0;
  This->FATStart2         = 0;
  This->DataStart         = 0;
...
)
{
  ...
  if (FileSystem->Master.NumFATs > 1)
  {
    ...
    FileSystem->Master.NumSectorsPerFAT;
  }
...
)
{
  ...
  (
    (Cluster - FileSystem->Master.RootCluster) *
...
static CLUSTER FindFreeCluster (LPFATFILESYSTEM FileSystem)
{
  U32       Buffer [SECTOR_SIZE / sizeof(U32)];
  IOCONTROL Control;
  CLUSTER   NewCluster;
...
)
{
  ...
  while (1)
  {
    ...
    (
      CurrentCluster == 0 ||
      CurrentCluster >= FAT32_CLUSTER_RESERVED
    )
    {
      break;
    }
...
static CLUSTER ChainNewCluster (LPFATFILESYSTEM FileSystem, CLUSTER Cluster)
{
  U32       Buffer [SECTOR_SIZE / sizeof(U32)];
  IOCONTROL Control;
  U32       Result;
...
static U32 CreateFolder (LPFILEINFO File)
{
  ...
  FileSystem = (LPFATFILESYSTEM) File->FileSystem;
...
static LPFATFILE OpenFile (LPFILEINFO Find)
{
  ...
  FileSystem = (LPFATFILESYSTEM) Find->FileSystem;
...
U32 FATCommands (U32 Function, U32 Parameter)
{
  switch (Function)
  {
    ...
    case DF_FS_CREATEFOLDER  : return (U32) CreateFolder((LPFILEINFO) Parameter);
    case DF_FS_DELETEFOLDER  : return (U32) DeleteFolder((LPFILEINFO) Parameter);
    case DF_FS_RENAMEFOLDER  : return (U32) RenameFolder((LPFILEINFO) Parameter);
    ...
    case DF_FS_WRITE         : return (U32) WriteFile((LPFATFILE) Parameter);
  }
