...
static LIST MainDesktopChildren =
{
  NULL, NULL, NULL, 0,
  KernelMemAlloc,
  KernelMemFree,
...
BOOL ShowDesktop (LPDESKTOP This)
{
  ...
  for (Node = Kernel.Desktop->First, Order = 1; Node; Node = Node->Next)
  {
    ...
    if (Desktop == This) Desktop->Order = 0; else Desktop->Order = Order++;
  }
...
void DeleteWindow (LPWINDOW This)
{
  ...
  Task    = This->Task;       if (Task    == NULL) return;
  Process = Task->Process;    if (Process == NULL) return;
  Desktop = Process->Desktop; if (Desktop == NULL) return;
...
BOOL BringWindowToFront (HANDLE Handle)
{
  ...
  {
    That = (LPWINDOW) Node;
    if (That == This) That->Order = 0; else That->Order = Order++;
  }
...
HANDLE GetSystemBrush (U32 Index)
{
  switch (Index)
  {
    case SM_COLOR_DESKTOP       : return (HANDLE) &Brush_Desktop;
    case SM_COLOR_HIGHLIGHT     : return (HANDLE) &Brush_High;
    case SM_COLOR_NORMAL        : return (HANDLE) &Brush_Normal;
    case SM_COLOR_LIGHT_SHADOW  : return (HANDLE) &Brush_HiShadow;
    case SM_COLOR_DARK_SHADOW   : return (HANDLE) &Brush_LoShadow;
    case SM_COLOR_CLIENT        : return (HANDLE) &Brush_Client;
    case SM_COLOR_TEXT_NORMAL   : return (HANDLE) &Brush_Text_Normal;
    case SM_COLOR_TEXT_SELECTED : return (HANDLE) &Brush_Text_Select;
    case SM_COLOR_SELECTION     : return (HANDLE) &Brush_Selection;
    case SM_COLOR_TITLE_BAR     : return (HANDLE) &Brush_Title_Bar;
    case SM_COLOR_TITLE_BAR_2   : return (HANDLE) &Brush_Title_Bar_2;
    case SM_COLOR_TITLE_TEXT    : return (HANDLE) &Brush_Title_Text;
  }
...
HANDLE GetSystemPen (U32 Index)
{
  switch (Index)
  {
    case SM_COLOR_DESKTOP       : return (HANDLE) &Pen_Desktop;
    case SM_COLOR_HIGHLIGHT     : return (HANDLE) &Pen_High;
    case SM_COLOR_NORMAL        : return (HANDLE) &Pen_Normal;
    case SM_COLOR_LIGHT_SHADOW  : return (HANDLE) &Pen_HiShadow;
    case SM_COLOR_DARK_SHADOW   : return (HANDLE) &Pen_LoShadow;
    case SM_COLOR_CLIENT        : return (HANDLE) &Pen_Client;
    case SM_COLOR_TEXT_NORMAL   : return (HANDLE) &Pen_Text_Normal;
    case SM_COLOR_TEXT_SELECTED : return (HANDLE) &Pen_Text_Select;
    case SM_COLOR_SELECTION     : return (HANDLE) &Pen_Selection;
    case SM_COLOR_TITLE_BAR     : return (HANDLE) &Pen_Title_Bar;
    case SM_COLOR_TITLE_BAR_2   : return (HANDLE) &Pen_Title_Bar_2;
    case SM_COLOR_TITLE_TEXT    : return (HANDLE) &Pen_Title_Text;
  }
...
U32 DesktopWindowFunc (HANDLE Window, U32 Message, U32 Param1, U32 Param2)
{
  switch (Message)
  {
    ...
    case EWM_MOUSEMOVE :
    {
      ...
      {
        // DrawMouseCursor(GC, SIGNED(OldMouseX), SIGNED(OldMouseY), FALSE);
        // DrawMouseCursor(GC, MouseX, MouseY, TRUE);
        ReleaseWindowGC(GC);
      }
...
*/
    ...
    default : return DefWindowFunc(Window, Message, Param1, Param2);
  }
