...
static LPXFSFILE NewXFSFile
(
  LPXFSFILESYSTEM FileSystem,
...
)
{
  ...
  FileSystem->PageBuffer = (U8*) KernelMemAlloc
  (
...
)
{
  ...
  for
  (
    Component = (LPPATHNODE) List->First;
    Component != NULL;
        ...
        )
        {
          ...
          else
          {
            if (FileRec->Attributes & XFS_ATTR_FOLDER)
            {
              ...
              if
              (
                ReadCluster(FileSystem, FileLoc->PageCluster, FileSystem->PageBuffer) == FALSE
              ) goto Out_Error;
              ...
              if
              (
                ReadCluster(FileSystem, FileLoc->FileCluster, FileSystem->IOBuffer) == FALSE
              ) goto Out_Error;
...
U32 XFSCommands (U32 Function, U32 Parameter)
{
  switch (Function)
  {
    ...
    case DF_FS_OPENNEXT        : return (U32) OpenNext((LPXFSFILE) Parameter);
    case DF_FS_CLOSEFILE       : return (U32) CloseFile((LPXFSFILE) Parameter);
    case DF_FS_DELETEFILE      : return DF_ERROR_NOTIMPL;
    case DF_FS_RENAMEFILE      : return DF_ERROR_NOTIMPL;
    case DF_FS_READ            : return DF_ERROR_NOTIMPL;
    case DF_FS_WRITE           : return DF_ERROR_NOTIMPL;
    case DF_FS_CREATEPARTITION : return CreatePartition((LPPARTITION_CREATION) Parameter);
  }
