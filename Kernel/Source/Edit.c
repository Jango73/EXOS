...
void CheckPositions (LPEDITFILE File)
{
  ...
  while (File->Cursor.X <  MinX) { File->Left--; File->Cursor.X++; }
  while (File->Cursor.X >= MaxX) { File->Left++; File->Cursor.X--; }
  while (File->Cursor.Y <  MinY) { File->Top--;  File->Cursor.Y++; }
  while (File->Cursor.Y >= MaxY) { File->Top++;  File->Cursor.Y--; }
...
static I32 Loop (LPEDITCONTEXT Context)
{
  ...
  while (1)
  {
    if (PeekChar())
    {
      ...
      if (KeyCode.VirtualKey == VK_UP)
      {
        Context->Current->Cursor.Y--;
        DrawText(Context->Current);
      }
      else
      if (KeyCode.VirtualKey == VK_RIGHT)
      {
        Context->Current->Cursor.X++;
        DrawText(Context->Current);
      }
      else
      if (KeyCode.VirtualKey == VK_LEFT)
      {
        Context->Current->Cursor.X--;
        DrawText(Context->Current);
      }
      else
      if (KeyCode.VirtualKey == VK_HOME)
      {
        GotoStartOfLine(Context->Current);
        DrawText(Context->Current);
      }
      else
      if (KeyCode.VirtualKey == VK_END)
      {
