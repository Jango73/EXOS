...
BOOL GetExecutableInfo_EXOS (LPFILE File, LPEXECUTABLEINFO Info)
{
  ...
  if (Header.Signature != EXOS_SIGNATURE)
  {
    ...
    KernelPrint
    (
      "GetExecutableInfo_EXOS() : Bad signature (%08X)\n",
      Header.Signature
    );
#endif
  ...
  while (1)
  {
    ...
    if (Chunk.ID == EXOS_CHUNK_FIXUP)
    {
      ...
      for (c = 0; c < NumFixups; c++)
      {
        ...
        if (Fixup.Section & EXOS_FIXUP_SOURCE_DATA)
        {
        ...
        if (ItemAddress != NULL)
        {
          ...
          if (Fixup.Section & EXOS_FIXUP_DEST_DATA)
          {
...
BOOL CreateProcess (LPPROCESSINFO Info)
{
  ...
  *((U32*)Process->HeapBase) = ID_HEAP;
  ...
  TaskInfo.Func = (TASKFUNC)
  (CodeBase + (ExecutableInfo.EntryPoint - ExecutableInfo.CodeBase));
