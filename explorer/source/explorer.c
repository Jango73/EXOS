...
/*
  if (GetProp(Window, Prop_Over))
  {
    ...
    PostMessage(GetWindowParent(Window), EWM_COMMAND, MAKEU32(ID, BN_CLICKED), (U32) Window);
  }
*/
...
U32 ButtonFunc (HANDLE Window, U32 Message, U32 Param1, U32 Param2)
{
  switch (Message)
  {
    case EWM_CREATE : return OnButtonCreate(Window, Param1, Param2);
    case EWM_DRAW   : return OnButtonDraw(Window, Param1, Param2);

    case EWM_MOUSEDOWN :
    {
      switch (Param1)
      {
        case MB_LEFT : return OnButtonLeftButtonDown(Window, Param1, Param2);
      }
    }
    break;
    ...
    case EWM_MOUSEUP :
    {
      switch (Param1)
      {
        case MB_LEFT : return OnButtonLeftButtonUp(Window, Param1, Param2);
      }
    }
    break;
...
U32 DesktopTask (LPVOID Param)
{
  ...
  while (1)
  {
    ...
    if (NewMouseButtons != MouseButtons)
    {
      ...
      {
        if (NewMouseButtons & MB_LEFT) DownButtons |= MB_LEFT;
        else UpButtons |= MB_LEFT;
      }

      if ((MouseButtons & MB_RIGHT) != (NewMouseButtons & MB_RIGHT))
      {
        if (NewMouseButtons & MB_RIGHT) DownButtons |= MB_RIGHT;
        else UpButtons |= MB_RIGHT;
      }

      if ((MouseButtons & MB_MIDDLE) != (NewMouseButtons & MB_MIDDLE))
      {
        if (NewMouseButtons & MB_MIDDLE) DownButtons |= MB_MIDDLE;
        else UpButtons |= MB_MIDDLE;
      }

      MouseButtons = NewMouseButtons;

      // if (DownButtons) PostMessage(Window, EWM_MOUSEDOWN, DownButtons, 0);
      // if (UpButtons)   PostMessage(Window, EWM_MOUSEUP, UpButtons, 0);
