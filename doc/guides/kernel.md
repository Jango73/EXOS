...
- Same-user process targeting policy and caller privilege resolution are centralized in `utils/ProcessAccess`: a process may target itself, processes owned by the same effective user, or any process when the caller resolves to administrator or kernel privilege. The same module also resolves owner processes for process-backed kernel objects such as tasks, windows, and desktops so generic handle-based syscalls can reuse the same decision path. These helpers are reused by exposure checks, process/task syscalls, generic handle-based syscalls, and shell inspection commands (`kernel/source/utils/ProcessAccess.c`, `kernel/source/expose/Expose-Security.c`, `kernel/source/SYSCall.c`, `kernel/source/shell/Shell-Commands-System.c`).
