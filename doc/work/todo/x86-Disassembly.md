...
## 1. Assess current x86-32 assembler/disassembler
- [x] **Opcode tables and instruction descriptors.** The 512-entry `Opcode_Table` and 80-entry `Extension_Table` live in
  `kernel/source/arch/intel/x86-32-Asm-Tables.c` and are exposed through `INTEL_OPCODE_PROTOTYPE` records declared in
  `kernel/include/arch/intel/x86-32-Asm.h`. The tables already cover 16/32-bit integer opcodes but intentionally exclude
  x87/MMX/SSE families, so any 64-bit port must either extend the tables or load a parallel set for long-mode opcodes.
- [x] **Decoder/encoder core.** `Intel_MachineCodeToStructure()` performs prefix handling, ModR/M dispatch, and operand decoding
...
- [x] **32-bit-centric assumptions.** Register metadata and operand formatting stop at the legacy set: `Intel_RegNames`
  ...
  rules.
- [x] **Reusable building blocks.** Tokenising/formatting helpers such as `Intel_PrintTypeSpec()` and
...
- [x] **Single-file expansion analysis.** `x86-32-Asm.c` is already >1,600 lines combining decoding, encoding, formatting, and
...
- [x] **Recommended split.** Introduce a dedicated `x86-64-Asm.c` that:
...
- [x] **Rationale.** Keeping ISA-specific control flows in separate translation units limits the risk of mixing REX logic with
...
- [ ] Ensure 0x66 operand-size override handling is centralized so both x86-32 and x86-64 paths reuse the same logic.
...
- [ ] Implement encoding rules for 64-bit instructions, leveraging existing x86-32 encoding where unchanged.
...
- [ ] Implement pretty-printing logic for 64-bit registers and immediates, reusing x86-32 formatting helpers.
- [ ] Ensure operand-size overrides (0x66) delegate to the shared x86-32 routines when decoding legacy forms.
...
- [ ] Document the final architecture decisions in `doc/guides/kernel.md` once implementation lands.
