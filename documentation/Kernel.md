...
All reusable helpers -such as the command line editor, adaptive delay, string containers, CRC/SHA-256 utilities, notifications, path helpers, TOML parsing, UUID support, regex, hysteresis control, cooldown timing, rate limiting, and network checksum helpers— live under `kernel/source/utils` with their public headers in `kernel/include/utils`. SHA-256 is exposed through `utils/Crypt` and bridged to the vendored BearSSL hash implementation under `third/bearssl`. This keeps generic infrastructure separated from core subsystems and makes it easier to share common code across the kernel.
