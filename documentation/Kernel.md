...
Desktop cursor runtime state is tracked per desktop (`DESKTOP` fields) and managed by `kernel/source/desktop/Desktop-Cursor.c` (position, visibility, clipping rectangle, active path, fallback reason). Software cursor overlay rendering is emitted as a final draw pass independent from window decoration mode, so pointer visibility does not depend on userland chrome policy.
