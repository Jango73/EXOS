...
An internal kernel test module (`kernel/source/desktop/Desktop-InternalTest.c`, `kernel/include/desktop/Desktop-InternalTest.h`) runs on `desktop show` and ensures two visible test windows exist on `MainDesktop`, with different sizes and internal title tags for deterministic windowing validation.
...
Desktop cursor runtime state is tracked per desktop (`DESKTOP` fields) and managed by `kernel/source/desktop/Desktop-Cursor.c` (position, pending target position, software-dirty state, visibility, clipping rectangle, active path, fallback reason). Software cursor overlay rendering is emitted as a final draw pass independent from window decoration mode, so pointer visibility does not depend on userland chrome policy. Software-path mouse movement invalidates bounded old/new cursor rectangles across intersecting visible windows instead of broadcasting full desktop redraw.
