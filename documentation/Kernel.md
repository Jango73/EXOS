...
Mounted filesystems expose their backing disk through `FILESYSTEM.PhysicalDisk`, with `FileSystemGetPhysicalDisk()` and `FileSystemHasPhysicalDisk()` providing a stable API for callers that need the associated `PHYSICALDISK` without relying on driver-specific struct layouts.
