...
- Remove all direct knowledge between `kernel/source/desktop/` and `kernel/source/desktop/components/`.
- Restrict `desktop/components/` to high-level desktop/window APIs only.
...
- `desktop/` must ignore everything in `desktop/components/`.
- `desktop/components/` must ignore all desktop internals and use only public high-level APIs.
...
- [ ] Confirm that no file in `desktop/` includes or references symbols from `desktop/components/`.
