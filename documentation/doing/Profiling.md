...
- Keep profiling behind a build-time flag (PROFILING) so non-profile builds pay zero cost.

## Step-by-step plan
1. [x] **Define config switch**
   - Add a global config toggle (PROFILING) in an existing config header used by both arches.
   - Default OFF; enable with `make PROFILING=1` so profiling code (and shell command) compiles in.
...
2. [x] **Add profiling API (shared)**
   ...
   - Provide no-op inline stubs when PROFILING is not defined.
...
3. [x] **Time source**
   - Use PIT counter latch (sub-ms) combined with system clock for elapsed time; store durations in microseconds.
...
7. [ ] **Architecture build wiring**
   - Update both script sets to expose a profiling build target:
     - i386: add/clone a script (e.g., `./scripts/i386/4-7-build-profile.sh`) enabling PROFILING.
