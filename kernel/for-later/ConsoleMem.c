...
static uint16_t map_passthrough(ConsoleDriver* dev, uint16_t a) { (void)dev; return a; }
...
static void mem_put_run(ConsoleDriver* dev, int x, int y, const ConsoleCell* cells, size_t count) {
	...
	if (x < 0) { size_t skip = (size_t)(-x); if (skip >= count) return; cells += skip; count -= skip; x = 0; }
...
static void mem_set_cursor(ConsoleDriver* dev, int x, int y, bool visible) {
	(void)dev; (void)x; (void)y; (void)visible; /* No hardware cursor */
}

static void mem_present(ConsoleDriver* dev) {
	(void)dev; /* No-op */
}
...
bool console_mem_init(ConsoleDriver* dev, int cols, int rows) {
	...
	if (!st->buf) { free(st); return false; }
