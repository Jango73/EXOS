...
void console_host_set_cursor(ConsoleHost* host, int x, int y, bool visible) {
    if (x < 0) x = 0;
    if (x >= host->cols) x = host->cols - 1;
    if (y < 0) y = 0;
    if (y >= host->rows) y = host->rows - 1;
    host->cur_x = x;
    host->cur_y = y;
...
void console_host_set_attr(ConsoleHost* host, uint16_t attr) { host->cur_attr = attr; }
...
void console_host_present(ConsoleHost* host) { host->dev->ops->present(host->dev); }
