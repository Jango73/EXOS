...
typedef struct tag_SEGMENTDESCRIPTOR
{
  ...
  U32 CanWrite      :  1;  // Read-only for data segments, Exec-Only for code segments
  U32 ConformExpand :  1;  // Conforming for code segments, expand-down for data segments
...
( \
  NULL32 | \
  ((((U32)(PSD)->Base_00_15) & 0xFFFF) << 0x00) | \
  ((((U32)(PSD)->Base_16_23) & 0x00FF) << 0x10) | \
  ((((U32)(PSD)->Base_24_31) & 0x00FF) << 0x18) \
)

#define SEGMENTGRANULAR(PSD) \
( \
  ((PSD)->Granularity == 0x00) ? N_1B : N_4KB \
)

#define SEGMENTLIMIT(PSD) \
( \
  NULL32 | \
  ((((U32)(PSD)->Limit_00_15) & 0xFFFF) << 0x00) | \
