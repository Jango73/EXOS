...
void SectorToBlockParams
(
  LPDISKGEOMETRY Geometry,
  U32 Sector,
  LPBLOCKPARAMS Block
)
{
...
void DriveOut
(
  U32 Port, U32 Drive, U32 Command, U8* Buffer,
  U32 Cylinder, U32 Head, U32 Sector, U32 Count
)
{
  U32 Flags;
...
static U32 Read (LPIOCONTROL Control)
{
  ...
  for (Current = 0; Current < Control->NumSectors; Current++)
  {
    ...
    if (Index != MAX_U32)
    {
      MemoryCopy
      (
        ((U8*) Control->Buffer) + (Current * SECTOR_SIZE),
        Disk->Buffer[Index].Data,
        SECTOR_SIZE
      );
    }
    else
...
static U32 GetInfo (LPDISKINFO Info)
{
  ...
  Info->Removable  = 0;
  Info->NumSectors = Disk->Geometry.Cylinders *
                     Disk->Geometry.Heads *
...
U32 StdHardDiskCommands (U32 Function, U32 Parameter)
{
  switch (Function)
  {
    case DF_LOAD           : return HardDiskInitialize();
