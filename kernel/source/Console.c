...
static void VarKernelPrintNumber
(I32 Number, I32 Base, I32 FieldWidth, I32 Precision, I32 Flags)
{
  STR Text [128];
...
Repeat :
    ...
    if (IsNumeric(*Format)) FieldWidth = SkipAToI(&Format);
    else
    if (*Format == '*')
    {
      Format++;
    ...
    {
      Format++;
      if (IsNumeric(*Format)) Precision = SkipAToI(&Format);
      else if (*Format == '*')
      {
        Format++;
    ...
    switch (*Format)
    {
      ...
      case 'p' :
        ...
        VarKernelPrintNumber
        ((U32) VarArg(Args, LPVOID), 16, FieldWidth, Precision, Flags);
        continue;
...
*/
      ...
      case 'o' : Base = 8; break;
      case 'X' : Flags |= PF_LARGE;
      case 'x' : Base = 16; break;
      case 'b' : Base = 2; break;
      case 'd' :
      case 'i' : Flags |= PF_SIGN;
      case 'u' : break;
      default  :
        if (*Format != '%') ConsolePrintChar('%');
        if (*Format) ConsolePrintChar(*Format);
        else Format--;
        continue;
    }
    ...
    if (Qualifier == 'h')
    {
      if (Flags & PF_SIGN) Number = VarArg(Args, I16);
      else Number = VarArg(Args, U16);
    }
    else
    {
      if (Flags & PF_SIGN) Number = VarArg(Args, INT);
      else Number = VarArg(Args, UINT);
    }
...
BOOL ConsoleGetString (LPSTR Buffer, U32 Size)
{
  ...
  while (Done == 0)
  {
    if (PeekChar())
    {
      ...
      if (KeyCode.VirtualKey == VK_BACKSPACE)
      {
        if (Index)
        {
          Index--;
      ...
      if (KeyCode.VirtualKey == VK_ENTER)
      {
