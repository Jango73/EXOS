...
LPSTR StringToLower (LPSTR Src)
{
  ...
  if (SrcPtr)
  {
    while (*SrcPtr) { *SrcPtr = CharToLower(*SrcPtr); SrcPtr++; }
  }
...
LPSTR StringToUpper (LPSTR Src)
{
  ...
  if (SrcPtr)
  {
    while (*SrcPtr) { *SrcPtr = CharToUpper(*SrcPtr); SrcPtr++; }
  }
...
U32 HexStringToU32 (LPCSTR Text)
{
  ...
  Text += 2; Length = StringLength(Text);
  ...
  for (c = 0, Value = 0, Shift = 4 * (Length - 1); c < Length; c++)
  {
    ...
    for (d = 0; d < 16; d++)
    {
      if (Text[c] == HexDigitLo[d]) { Temp = d; FoundDigit = 1; break; }
      if (Text[c] == HexDigitHi[d]) { Temp = d; FoundDigit = 1; break; }
    }
...
(LPSTR Text, I32 Number, I32 Base, I32 Size, I32 Precision, I32 Type)
{
  STR c, Sign, Temp [66];
  ...
  if (Type & PF_SIGN)
  {
    if (Number < 0) { Sign = '-'; Number = -Number; Size--; }
    else
    if (Type & PF_PLUS) { Sign = '+'; Size--; }
    else
    if (Type & PF_SPACE) { Sign = ' '; Size--; }
  }
