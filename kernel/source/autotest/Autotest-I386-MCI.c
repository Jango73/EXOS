/************************************************************************\
    ...
    I386 Machine Code Instruction Disassembler Tests
...
static DISASM_TEST DisasmTests[] = {
    ...
    { {0x06}, 1, TEXT("????"), TEXT("Invalid opcode (removed in i386)") },
...
BOOL TestI386Disassembler(void)
{
    ...
    KernelLogText(LOG_DEBUG, TEXT("Starting I386 Disassembler Tests..."));
    ...
    for (U32 i = 0; i < NumDisasmTests; i++)
    {
        ...
        U32 Length = Intel_MachineCodeToString(
            (LPCSTR)Test->MachineCode, 
            (LPCSTR)Test->MachineCode, 
            ResultBuffer
        );
        ...
        {
            KernelLogText(LOG_DEBUG, TEXT("Test %d PASSED: %s -> %s"), i, Test->Description, ResultBuffer);
            PassedTests++;
        }
        else
        {
            KernelLogText(LOG_DEBUG, TEXT("Test %d FAILED: %s"), i, Test->Description);
            if (!LengthOK)
            {
                KernelLogText(LOG_DEBUG, TEXT("  Length mismatch: expected %d, got %d"), Test->Length, Length);
            }
            if (!AssemblyOK)
            {
                KernelLogText(LOG_DEBUG, TEXT("  Assembly mismatch: expected '%s', got '%s'"), Test->ExpectedAsm, ResultBuffer);
            }
            FailedTests++;
            AllPassed = FALSE;
        }
    ...
    KernelLogText(LOG_DEBUG, TEXT("I386 Disassembler Tests Complete: %d passed, %d failed"), PassedTests, FailedTests);
