...
void TestI386Disassembler(TEST_RESULTS* Results) {
    ...
    for (U32 i = 0; i < NumDisasmTests; i++) {
        ...
        if (LengthOK && AssemblyOK) {
            ...
            DEBUG(TEXT("[TestI386Disassembler] Test %d FAILED: %s"), i, Test->Description);
            if (!LengthOK) {
                DEBUG(TEXT("[TestI386Disassembler]   Length mismatch: expected %d, got %d"), Test->Length, Length);
            }
            if (!AssemblyOK) {
                DEBUG(TEXT("[TestI386Disassembler]   Assembly mismatch: expected '%s', got '%s'"), Test->ExpectedAsm, ResultBuffer);
            }
        }
    }
}
