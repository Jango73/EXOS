...
/*
          if (TaskList.Current == &KernelTask)
          {
            ...
            LPTASK Task;
            U32 Value;
            for (Node = Kernel.Task->First, Index = 0; Node; Node = Node->Next)
            {
              Task = (LPTASK) Node;

              switch (Task->Status)
              {
                case TASK_STATUS_DEAD     : Value = 0x00FFFFFF; break;
                case TASK_STATUS_RUNNING  : Value = 0x0000FF00; break;
                case TASK_STATUS_SLEEPING : Value = 0x000000FF; break;
                case TASK_STATUS_WAITING  : Value = 0x00FF0000; break;
              }
