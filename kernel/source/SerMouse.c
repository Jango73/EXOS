...
static BOOL WaitMouseData (U32 TimeOut)
{
  ...
  while (TimeOut)
  {
    ...
    if
    (
      (Status & SERIAL_LSR_OE) ||
      (Status & SERIAL_LSR_PE) ||
      (Status & SERIAL_LSR_FE) ||
...
static U32 MouseInitialize ()
{
  ...
  Byte = InPortByte(MOUSE_PORT + SERIAL_DATA); Delay();
  Byte = InPortByte(MOUSE_PORT + SERIAL_DATA); Delay();
  Byte = InPortByte(MOUSE_PORT + SERIAL_DATA); Delay();
  Byte = InPortByte(MOUSE_PORT + SERIAL_DATA); Delay();
  Byte = InPortByte(MOUSE_PORT + SERIAL_DATA); Delay();
...
void MouseHandler_Microsoft ()
{
  ...
  if ((Buttons & BIT_6) != BIT_6)
  {
    ...
    */
    ...
    InPortByte(MOUSE_PORT + SERIAL_DATA); Delay();
    InPortByte(MOUSE_PORT + SERIAL_DATA); Delay();
    InPortByte(MOUSE_PORT + SERIAL_DATA); Delay();
    InPortByte(MOUSE_PORT + SERIAL_DATA); Delay();
...
U32 SerialMouseCommands (U32 Function, U32 Parameter)
{
  switch (Function)
  {
    case DF_LOAD             : return (U32) MouseInitialize();
    case DF_GETVERSION       : return MAKE_VERSION(VER_MAJOR, VER_MINOR);
    case DF_MOUSE_RESET      : return 0;
    case DF_MOUSE_GETDELTAX  : return (U32) GetDeltaX();
    case DF_MOUSE_GETDELTAY  : return (U32) GetDeltaY();
    case DF_MOUSE_GETBUTTONS : return (U32) GetButtons();
  }
