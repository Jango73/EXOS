...
static void ListFile (LPFILE File)
{
  ...
  if (File->Attributes & FS_ATTR_READONLY) KernelPrint(TEXT("R")); else KernelPrint(TEXT("-"));
  if (File->Attributes & FS_ATTR_HIDDEN)   KernelPrint(TEXT("H")); else KernelPrint(TEXT("-"));
  if (File->Attributes & FS_ATTR_SYSTEM)   KernelPrint(TEXT("S")); else KernelPrint(TEXT("-"));
...
static BOOL ParseCommand (LPSHELLCONTEXT Context)
{
  ...
  for (Index = 0; COMMANDS[Index].Command != NULL; Index++)
  {
    if
    (
      StringCompareNC(Context->Command, COMMANDS[Index].Name) == 0 ||
