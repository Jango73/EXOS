...
# Example: src2addr_convert "boot-mbr/bin/payload.elf" "boot-mbr/source/vbr-payload-c.c:407" "-M i8086"
...
function src2addr_convert() {
    ...
    END {
        if (count > 1) {
            ...
            print "Use ./scripts/linux/utils/show-x86-32.sh or ./scripts/linux/utils/show-x86-64.sh 0x" first_addr " to see disassembly context"
        } else if (count == 1) {
            print ""
            print "Use ./scripts/linux/utils/show-x86-32.sh or ./scripts/linux/utils/show-x86-64.sh 0x" first_addr " to see disassembly context"
