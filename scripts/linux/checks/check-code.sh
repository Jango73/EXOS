...
HEADER_TEMPLATE="$SCRIPT_DIR/../../../doc/shared/Code-Header.txt"
...
if command -v git >/dev/null 2>&1; then
    FILES=$(git ls-files '*.c' '*.h' ':(exclude)third/**' ':(exclude)doc/**')
    SCAN_MODE="tracked .c/.h files (excluding third/ and doc/)"
else
    FILES=$(find . \
        -path './.git' -prune -o \
        -path './third' -prune -o \
    ...
    SCAN_MODE="filesystem .c/.h files (excluding .git/, third/ and doc/)"
fi
