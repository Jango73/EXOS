...
```bash
sudo scripts/net/setup-local-netboot.sh --arch x86-64
```
...
```bash
sudo scripts/net/setup-local-netboot.sh \
    --arch x86-64 \
...
```bash
./scripts/build --arch x86-64 --fs ext2 --debug --split --uefi
```

## 3) Deploy boot files to HTTP slot

```bash
sudo scripts/net/deploy-local-netboot.sh --arch x86-64 --slot latest --rotate
```
