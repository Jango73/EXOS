...
qemu-system-i386 \
-drive format=raw,file="$IMG_1_PATH" -drive format=raw,file="$IMG_2_PATH" \
-serial file:"log/debug-com1.log" -serial file:"log/debug-com2.log" \
-no-reboot -nographic -boot d \
-D log/qemu-trace.log \
-d guest_errors,pcall,cpu_reset \
-trace get_mem_fault_cpu_index \
-trace postcopy_pause_fault_thread \
-monitor stdio
