...
IMG_PATH="build/image/${BUILD_IMAGE_NAME}/boot-hd/exos.img"
...
if [ ! -f "$IMG_PATH" ]; then
    ...
    echo "Build first: bash scripts/build.sh --arch $ARCH --fs $FS --$CONFIG"
    exit 1
fi

source scripts/utils/smoke-test-common.sh
...
setsid /bin/bash -lc "MONITOR_PORT=$MONITOR_PORT scripts/run --arch $ARCH --fs $FS --$CONFIG" >"$RUN_LOG" 2>&1 &
QEMU_WRAPPER_PID=$!
