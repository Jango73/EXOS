...
if [ ! -f "$DEBUG_ELF" ] || [ ! -f "$IMG_PATH" ]; then
    echo "Missing build artifacts. Build first: ./scripts/build --arch x86-64 --fs ext2 --debug --uefi"
    exit 1
fi
...
OVMF_VARS_COPY="build/image/${BUILD_IMAGE_NAME}/boot-uefi/ovmf-vars-debug-f12.fd"
