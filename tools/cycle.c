...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "o:s:S:c:", long_opts, NULL)) != -1) {
        switch (opt) {
            ...
            case 's': bufsize = strtoul(optarg, NULL, 10); break;
            case 'S': pattern = optarg; break;
            case 'c': stop_count = atoi(optarg); break;
            default:
                fprintf(stderr,
                        "Usage: %s -o <file> [-s <size>] [-S <string> -c <count>]\n",
                        argv[0]);
                return 1;
        }
    }
    ...
    while ((r = read(STDIN_FILENO, chunk, sizeof(chunk))) > 0) {
        ...
        if (!stop_writing) {
            ...
            if (ftruncate(fd, 0) < 0) { perror("ftruncate"); break; }
            if (lseek(fd, 0, SEEK_SET) < 0) { perror("lseek"); break; }
            if (write(fd, buffer, buf_len) != (ssize_t)buf_len) { perror("write"); break; }
            fsync(fd);
        }
